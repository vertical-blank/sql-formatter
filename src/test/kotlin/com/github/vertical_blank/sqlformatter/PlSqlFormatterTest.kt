...
object PlSqlFormatterTest: Spek({
  val formatter = SqlFormatter.of(Dialect.PlSql)

  describe("PlSqlFormatter") { with(formatter) {
    ...
    it("formats FETCH FIRST like LIMIT") {
      expect(format("SELECT col1 FROM tbl ORDER BY col2 DESC FETCH FIRST 20 ROWS ONLY;")).toBe("""
        SELECT
          col1
        FROM
          tbl
    ...
    it("replaces ? numbered placeholders with param values") {
      val result = format("SELECT ?1, ?2, ?0;",
        mapOf(
          "0" to "first",
          "1" to "second",
          "2" to "third",
