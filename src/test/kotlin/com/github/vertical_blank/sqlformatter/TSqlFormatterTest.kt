...
object TSqlFormatterTest: Spek({
  val formatter = SqlFormatter.of(Dialect.TSql)

  describe("TSqlFormatter") { with(formatter) {
    behavesLikeSqlFormatter(formatter);
    supportsCase(formatter);
    supportsCreateTable(formatter);
    supportsAlterTable(formatter);
    supportsStrings(formatter, listOf(StringLiteral.DoubleQuote, StringLiteral.SingleQuote, StringLiteral.NSingleQuote, StringLiteral.Bracket))
    ...
    it("replaces @variables with param values") {
      val result = format("""SELECT @variable, @"var name1", @[var name2];""",
        mapOf(
