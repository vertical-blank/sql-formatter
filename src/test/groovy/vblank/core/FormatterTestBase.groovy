...
abstract class FormatterTestBase {
    ...
    @Test void "uses given indent config for indention"() {
    ...
    @Test void "formats simple SET SCHEMA queries"() {
    ...
    @Test void "formats simple SELECT query"() {
    ...
    @Test void "formats complex SELECT"() {
    ...
    @Test void "formats SELECT with complex WHERE"() {
        String result = SqlFormatter.format(
    ...
    @Test void "formats SELECT with toplevel reserved words"() {
        String result = format(
    ...
    @Test void "formats LIMIT with two comma-separated values on single line"() {
        String result = format(
            "LIMIT 5, 10;"
        )
        expect(result).toBe(
    ...
    @Test void "formats LIMIT of single value followed by another SELECT using commas"() {
        String result = format(
    ...
    @Test void "formats LIMIT of single value and OFFSET"() {
        String result = format(
            "LIMIT 5 OFFSET 8;"
        )
        expect(result).toBe(
    ...
    @Test void "recognizes LIMIT in lowercase"() {
        String result = format(
            "limit 5, 10;"
        )
        expect(result).toBe(
    ...
    @Test void "preserves case of keywords"() {
        String result = format(
    ...
    @Test void "formats SELECT query with SELECT query inside it"() {
        String result = format(
    ...
    @Test void "formats SELECT query with INNER JOIN"() {
        String result = format(
    ...
    @Test void "formats SELECT query with different comments"() {
        String result = format(
            "SELECT\n" +
                "/*\n" +
    ...
    @Test void "formats simple INSERT query"() {
        String result = format(
    ...
    @Test void "keeps short parenthized list with nested parenthesis on single line"() {
        String result = format(
            "SELECT (a + b * (c - NOW()))"
        )
        expect(result).toBe(
    ...
    @Test void "breaks long parenthized lists to multiple lines"() {
        String result = format(
    ...
    @Test void "formats simple UPDATE query"() {
        String result = format(
    ...
    @Test void "formats simple DELETE query"() {
        String result = format(
    ...
    @Test void "formats simple DROP query"() {
        String result = format(
    ...
    @Test void "formats uncomplete query"() {
    ...
    @Test void "formats query that ends with open comment"() {
        String result = format("SELECT count(*)\n/*Comment")
        expect(result).toBe(
            "SELECT\n" +
                "  count(*)\n" +
    ...
    @Test void "formats UPDATE query with AS part"() {
        String result = format(
    ...
    @Test void "formats top-level and newline multi-word reserved words with inconsistent spacing"() {
    ...
    @Test void "formats long double parenthized queries to multiple lines"() {
    ...
    @Test void "formats short double parenthized queries to one line"() {
        String result = format("((foo = 'bar'))")
        expect(result).toBe("((foo = 'bar'))")
    }

    @Test void "formats single-char operators"() {
    ...
    @Test void "formats multi-char operators"() {
        expect(format("foo != bar")).toBe("foo != bar")
        expect(format("foo <> bar")).toBe("foo <> bar")
    ...
    @Test void "formats logical operators"() {
    ...
    @Test void "formats AND OR operators"() {
    ...
    @Test void "recognizes escaped strings"() {
        expect(format("\"foo \\\" JOIN bar\"")).toBe("\"foo \\\" JOIN bar\"")
        expect(format("'foo \\' JOIN bar'")).toBe("'foo \\' JOIN bar'")
        expect(format("`foo `` JOIN bar`")).toBe("`foo `` JOIN bar`")
    }

    @Test void "formats postgres specific operators"() {
    ...
    @Test void "keeps separation between multiple statements"() {
